function compressed(s) {
	...
	while (i < s.length) {
		...
		let n = s.slice(i, index);
		let [str, num] = [s[index], n];
		let idx = 0;
		while (idx < num) {
			result.push(str);
			idx++;
...
//   return arr
// };
// let val = findAnagrams(str1,str2);
// let end = Date.now();
// console.log(end-start)
// console.log(val);

// let p = 'abcdcbafbac';
// const hash = p.split('').reduce((res, c) => {
//       if (!res[c]) {
//           res[c] = 0;
//       }
...
//     let indexAdjester = 'a'.charCodeAt(0);
//     let result = [];
//     for(let i = 0; i<p.length; i++){
//         let idx = p[i].charCodeAt(0) - indexAdjester;
//         pArr[idx]++;
//     }
//     let i = 0;
//     while(i<s.length-p.length){
...
//         let j = i;
//         while(j < i+p.length){
//             let idx = s[j].charCodeAt(0) - indexAdjester;
//             sArr[idx]++;
//             j++;
//         }
//         let pStr = pArr.join('');
...
// let symbol = ['a', 'b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q', 'r','s','t','u','v','w','x','y','z' ,'1','2', '3','4','5','6','7','8','9','0', ".","\\","/","?",'!',")","(",",","[","]","}", "{", ":", ";", "=","+","-","_","*","&","%","$","#","@","`","`","~"];
// let arr = new Array(symbol.length).fill(0);
// console.log(arr)
// let idxAdj = 'a'.charCodeAt(0);
// for(let char of symbol){
//   let idx = char.charCodeAt(0)-idxAdj;
//   if(idx<0){
//     // console.log(arr[idx])
//     idx = idx+idxAdj
//     arr[idx] = char;
...
var findAnagrams = function (s, p) {
	...
	while (i <= s.length - p.length) {
		...
		let j = i;
		while (j < i + p.length) {
			let idx = s[j].charCodeAt(0) - idxAdj;
			sArr[idx]++;
			j++;
		}
		let pStr = pArr.reduce((a, b) => a + b, "");
		let sStr = sArr.reduce((a, b) => a + b, "");
		if (pStr === sStr) {
			result.push(i);
		}
		i++;
